  ...
  <div class="row">
            ...
            <div id="mid">
              <div class="info">
                <p class="text-left" style="font-size: 14px">
                  ...
                  Srvd by: {{ $store.getters.userData.user.name }}
                </p>
              </div>
            </div>
            <!--End Invoice Mid-->
              ...
              <div id="table">
                <table>
                  ...
                  <tr v-if="totalCartValue">
                    ...
                    <td class="Rate" colspan="2"><h2>Total Amount</h2></td>
                    <td class="payment">
                      <h2>{{ Number(totalCartValue).toLocaleString() }}</h2>
                    </td>
                  </tr>
                  <tr>
                    <td></td>
                  ...
                  <tr v-if="special_discount">
                    ...
                    <td class="Rate" colspan="2"><h2>Special Discount</h2></td>
                    <td class="payment">
                      <h2>{{ Number(special_discount).toLocaleString() }}</h2>
                    </td>
                  </tr>
                  ...
                  <tr v-if="netAmountCalculate">
                    ...
                    <td class="Rate" colspan="2"><h2>Net Amount</h2></td>
                    <td class="payment">
                      <h2>{{ Number(netAmountCalculate).toLocaleString() }}</h2>
                    </td>
                  </tr>
                </table>
              </div>
    ...
    >
      <div class="modal-content scrollbar-width-thin barcodeReaderModal">
        ...
        <div class="modal-body" id="printMe">
          <!-- <ImageBarcodeReader @decode="onDecode" @error="onError" v-if="renderBarcodeReader"></ImageBarcodeReader> -->
          <StreamBarcodeReader
            @decode="(a,b,c) => onDecode(a,b,c)"
            @loaded="() => onLoaded()"
            v-if="renderBarcodeReader"
          ></StreamBarcodeReader>
        ...
        .then((res) => {
          ...
          if (res.status == 200) {
            ...
            this.$swal({
              html:
                "<h3 style='color:red'>" +
                this.pform.return_amount.toLocaleString() +
                "</h3>",
              title: "Return Amount",
              confirmButtonCategory: "#3085d6",
              cancelButtonCategory: "#d33",
  ...
  computed: {
    ...
    specialDiscount: function () {
      ...
      if (this.form.order_discount) {
        ...
        if (percentage.length > 1) {
          order_discount =
            (this.totalCartValue * parseFloat(percentage[0])) / 100;
        } else {
          order_discount = checkNumber
            ? parseFloat(this.form.order_discount)
            : 0;
        }
      }
    ...
    customer_discount: function () {
      let customer_discount =
        (this.totalCartValue * parseFloat(this.customer.discount)) / 100;
    ...
    customer_group_discount: function () {
      let customer_group_discount =
        (this.totalCartValue * parseFloat(this.customer.group_discount)) / 100;
    ...
    netAmountCalculate: function () {
      ...
      // let collection = this.pform.payments
      //   .map((item) => parseFloat(item.amount))
      //   .reduce((prev, curr) => prev + curr, 0);
      // var collection = this.pform.payments.reduce((total, item) => { return total + parseFloat(item.amount)}, 0);

      // console.log("collection_payments1", totalDiscount); 
      // console.log("collection_payments2", this.all_discount); 
      // console.log("collection_payments3", totalDiscount - this.all_discount); 

      // if(this.form.order_discount) {
      //   console.log("collection_payments 4", collection - (totalDiscount - this.all_discount)); 

      // }else{
      //   this.form.order_discount = 0;
      //   var new_collection = this.pform.payments.reduce((total, item) => { return total + parseFloat(item.amount)}, 0);

      //   console.log("collection_payments 5", new_collection); 

      // }
      
      this.pform.total_collect_amount = result;
