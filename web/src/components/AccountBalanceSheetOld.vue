<template>
    <tbody> 
        <tr :style="(account.account_type != 'ledger') ? 'font-weight: bold' : ''">
            ...
            <td class="text-right">{{ parseFloat(account.debit_amount) + parseFloat(account.credit_amount) }}</td>
        </tr>
    </tbody>               
    <AccountBalanceSheet v-if="hasChildren" v-for="(child, c) in account.children" :key="c" :account="child" :level="(level + 1)" :spaces="(spaces + 15)" :code_active="showCode"></AccountBalanceSheet>
</template>
