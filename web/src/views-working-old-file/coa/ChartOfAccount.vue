        ...
        onChangeAccountGroup(value) {
            ...
            if(group_id != '' || group_id != 0) {
                this.all_account_types.filter((titem) => {
                    ...
                    var selected_type_data = this.all_account_types.find(({id}) => id == this.form_type.id); 
                if(selected_type_data.class_id != group_id) {
                    this.fetchTypeCode(group_id, "group");
                }else{ 
                    if(this.modalAccSubTypeActive) { 
                        this.fetchTypeCode(group_id, "group");
                    }else {    
                        this.form_type.type_code = selected_type_data.type_code; 
                    }
                }
