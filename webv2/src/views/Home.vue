...
export default {
  ...
  computed: {
    permission() {
      ...
      const userPermissions = this.$store.getters.userAllPermissions;
      
      // If permissions haven't loaded yet, return empty
      if (!userPermissions || Object.keys(userPermissions).length === 0) {
        return "";
      }

      if (this.$route.meta.parent_module) {
        data = userPermissions?.[pname]?.[0]?.children?.[path_name] || "";
      } else {
        data = userPermissions?.[module_name]?.[0]?.other_actions || "";
      }
      
      return data;
    },
    
    // Add a computed property to check if permissions are loaded
    permissionsLoaded() {
      const userPermissions = this.$store.getters.userAllPermissions;
