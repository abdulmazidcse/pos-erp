...
export default {
  ...
  setup() {
    ...
    const convartRate = computed(() => {
      const cartPriceRate = state.points_setting?.cart_price_rate ?? 1;  // fallback 1 or any default
      const cartPointsRate = state.points_setting?.cart_points_rate ?? 1; // fallback 1 or any default
      ...
      if (extraMoney < 0) { 
        ...
        if(!this.checkItem('cash')){
          ...
          if(checkCollectionAmount >= this.pform.grand_total){
            ...
            this.$toast.warning('Collection amount  ee e violate');
            this.isSubmit = false;
            this.btn_disabled = false;
            return false;
          } else{
