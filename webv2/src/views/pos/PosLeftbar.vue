  ...
  <div class="row">
        ...
        <div>
          ...
          >
            <thead class="tableFloatingHeaderOriginal">
              ...
              <tr class="item-head">
                <th>Total Item</th>
                <th>{{ Number(getCartItems().length).toLocaleString() }}</th>
              </tr>
            </thead>
          </table>
        </div>
    ...
    <Modal  @close="addConfirmModal()" :modalActive="confirmModalActive">
      <div class="modal-content scrollbar-width-thin confirm-window">
        ...
        <div class="modal-body inv-perview" v-if="getCartItems().length > 0">
            ...
            <div id="bot">
              <div id="table">
                <table >
                  ...
                  <tr class="service " v-for="(item, i) in getCartItems()" v-if="getCartItems().length > 0" :key="i" >
...
import { ref, reactive, computed, toRaw, inject, onMounted, getCurrentInstance, onBeforeUnmount, watch } from "vue";
...
export default {
  ...
  setup() {
    ...
    // Refs for form states
    const isSubmit = ref(false);
    const couponSubmit = ref(false);
    const invDisabled = ref(false);
    const editMode = ref(false);
    const enabledDiscount = ref(false);
    // const submitDisable = ref(true);
    const btn_disabled = ref(false);
    const special_discount = ref(0);
    const errors = ref("");
    const popupError = ref("");
    const discountOverwrite = ref(0);
    ...
    const customer_points = ref(0);
    const items = ref([]);
    const outlets = ref([]);
    const units = ref([]);
    const more_payemnts = ref([]);
    const selecteditem = ref("");
    const searchIteam = ref("");
    const arrowCounter = ref(-1);
    const searchProduct = ref([]);
    const singleProductItem = ref("");
    const couponDiscoun = ref(0);
    const eventTimeForScanner = ref("");
    const barcode = ref("");
    const returnInfo = ref("");
    const return_replace = ref([]);
    const sale_return_info = ref({ return_amount: "", return_reason: "" });
    const renderBarcodeReader = ref(false);
    const barcoderRrenderCounter = ref(0); 
    ...
    const cform = reactive(new Form({
      id: "",
      customer_code: "C" + Math.floor(Math.random() * 100000),
      company_id: 0,
      customer_group_id: 1,
      name: "",
      email: "",
    ...
    //   console.log('🔄 cartItems computed triggered:', items.length, 'items');
    //   return items;
    // });
    ...
    //   return [...items]; // This creates a NEW array every time
    // });

    // const cartItems = computed(() => {
    //   const items = store.getters.cartItems || [];
    //   // Break the Proxy reference completely
    ...
    const convartRate = computed(() => {
      const cartPriceRate = state.points_setting?.cart_price_rate ?? 1;
      const cartPointsRate = state.points_setting?.cart_points_rate ?? 1;
      return cartPriceRate / cartPointsRate;
    });
    ...
    const totalItemDiscountValue = computed(() => {
      return store.getters.cartItems.reduce(function (total, item) {
        let subtotal = 0;
    ...
    const totalItemNetValue = computed(() => {
      return store.getters.cartItems.reduce(function (total, item) {
        let results;
        if (item.uom == 5) {
    ...
    const specialDiscount = computed(() => {
      let order_discount = 0;
      let checkNumber = !isNaN(form.order_discount);
      if (form.order_discount) {
        let percentage = form.order_discount.split("%");
        if (percentage.length > 1) {
          order_discount = (totalCartValue.value * parseFloat(percentage[0])) / 100;
        } else {
          order_discount = checkNumber ? parseFloat(form.order_discount) : 0;
        }
      }
      return order_discount;
    });

    const invoiceVat = computed(() => {
      let order_vat = 0;
      let checkNumber = !isNaN(form.order_vat);
      if (form.order_vat) {
        let percentage = form.order_vat.split("%");
        if (percentage.length > 1) {
          order_vat = (totalCartValue.value * parseFloat(percentage[0])) / 100;
        } else {
          order_vat = checkNumber
            ? (totalCartValue.value * parseFloat(form.order_vat)) / 100
            : 0;
        }
      }
      return order_vat;
    ...
    const customer_discount = computed(() => {
      if (customer.value) {
        let customer_discount = (totalCartValue.value * parseFloat(customer.value.discount)) / 100;
    ...
    const customer_group_discount = computed(() => {
      if (customer.value) {
        let customer_group_discount = (totalCartValue.value * parseFloat(customer.value.group_discount)) / 100;
    ...
    const totalCollections = computed(() => {
      return pform.payments.reduce(function (total, item) {
        return total + parseFloat(item.amount);
      }, 0);
    });
    const netAmountCalculate = computed(() => {
        let totalDiscount = all_discount.value + specialDiscount.value + customer_group_discount.value + customer_discount.value + couponDiscoun.value; 
        let totalVatTax = invoiceVat.value + totalCartTax.value;
        let result = parseFloat(totalCartValue.value + totalVatTax - totalDiscount).toFixed(2); 
        pform.grand_total = result;        
        pform.order_items_vat = totalCartTax.value;
        pform.total_amount = parseFloat(totalCartValue.value).toFixed(2);
        pform.payments[0].amount = parseFloat(result).toFixed(2); 
        return parseFloat(result).toFixed(2);
    });

    const salesReturnAmountSumVal = computed(() => {
      let sumTotal = return_replace.value.reduce(function (total, ritem) {
    ...
    const selectProduct = (item) => {
      searchIteam.value = item.product_name + " (" + item.product_code + ")";
    };

    const setResult = (item) => {
      store.dispatch("addCartItem", item);
      searchIteam.value = "";
      searchProduct.value.splice(0);
    };

    const addProductToCart = () => {
      if (singleProductItem.value != "") {
        store.dispatch("addCartItem", singleProductItem.value);
      } else {
        toast.warning("Product not found!");
      }
      singleProductItem.value = "";
      searchIteam.value = "";
      searchProduct.value = [];
    };
    ...
    const inputChanged = (event) => {
      setTimeout(() => {
        ...
        if (searchIteam.value == "") return;

        var checkNumber = searchIteam.value.toLowerCase();
        var first2digit = String(checkNumber).slice(0, 2);
        var weightFigure = "";
        var product_weight = "";
        if (Number(first2digit) == 99) {
          searchIteam.value = String(checkNumber).slice(2, 7);
          weightFigure = String(checkNumber).slice(-6, -1);
          product_weight = parseFloat(Number(weightFigure) / 1000).toFixed(3);
        }

        var filtered = store.getters.productItems.filter((item) => {
          let item_data = item.product_name + " (" + item.product_code + ")";
          return item_data.toLowerCase().match(searchIteam.value.toLowerCase());
        });
        ...
        searchProduct.value.push(...filtered);
        let match_data = searchProduct.value[arrowCounter.value]
          ? searchProduct.value[arrowCounter.value].product_name +
            " (" +
            searchProduct.value[arrowCounter.value].product_code +
            ")"
          : "";
        if (match_data != searchIteam.value) {
    ...
    const onArrowDown = (event) => {
      if (searchProduct.value.length > 0) {
        arrowCounter.value =
          event.code == "ArrowDown" ? ++arrowCounter.value : --arrowCounter.value;
        if (arrowCounter.value >= searchProduct.value.length)
          arrowCounter.value = arrowCounter.value % searchProduct.value.length;
        else if (arrowCounter.value < 0)
          arrowCounter.value = searchProduct.value.length + arrowCounter.value;
        selectProduct(searchProduct.value[arrowCounter.value]);
        singleProductItem.value = searchProduct.value[arrowCounter.value];
      }
    };
    ...
    const onArrowUp = (event) => {
      if (searchProduct.value.length > 0) {
        arrowCounter.value =
          event.code == "ArrowUp" ? --arrowCounter.value : ++arrowCounter.value;
        if (arrowCounter.value >= searchProduct.value.length)
          arrowCounter.value = arrowCounter.value % searchProduct.value.length;
        else if (arrowCounter.value < 0)
          arrowCounter.value = searchProduct.value.length + arrowCounter.value;
    ...
    const selectCustomer = (event) => {
      if (event) {
        customer.value = state.customers.find((e) => e.value == event);
        pform.customer_id = customer.value.value;
        pform.customer_group_id = customer.value.customer_group_id;
        pform.customer_group_name = customer.value.customer_group_name;
        if (pform.customer_id == 1) {
          // This will trigger the computed property update
        }
      }
    };

    // Watch for sale_return_info.return_amount changes
    watch(() => sale_return_info.value.return_amount, () => {
      sale_return_info.value.return_amount = return_replace.value.reduce(function (total, ritem) {
        return total + ritem.replace_mrp_price * ritem.return_qty;
      }, 0);
    });

    const updateCartItems = () => {
      // Completely break ALL reactivity
      const storeItems = store.getters.cartItems;
      const plainArray = [];
      
      if (storeItems && storeItems.length > 0) {
        for (let i = 0; i < storeItems.length; i++) {
          // Create plain objects without any reactivity
          plainArray.push({
            product_id: storeItems[i].product_id,
            product_stock_id: storeItems[i].product_stock_id,
            product_name: storeItems[i].product_name,
            mrp_price: storeItems[i].mrp_price,
            product_code: storeItems[i].product_code,
            quantity: storeItems[i].quantity,
            stock_quantity: storeItems[i].stock_quantity,
            uom: storeItems[i].uom,
            weight: storeItems[i].weight,
            discount: storeItems[i].discount,
            discount_percent: storeItems[i].discount_percent,
            expires_date: storeItems[i].expires_date
            // Add other properties you need
          });
        }
      }
      
      cartItems.value = plainArray;
    //   console.log('🔄 Pure JS array:', cartItems.value);
    };
    ...
    onMounted(async () => {
        ...
        const res = await axios.get(app2.apiUrl + "/sale-helper", app2.headers);
        ...
        const handleResize = () => {
            ...
            state.winHeight = window.innerHeight - 160;
            // console.log('resized');
            // ... rest of resize logic
        };

        // Call once immediately if needed
        handleResize();

        // ✅ Attach listener to real window
        ...
        const handleKeydown = (e) => {
            ...
            if (!submitDisable.value) {
                ...
                toast.warning("Collection amount can't be less than net amount!");
            }
            }

            if (e.shiftKey && e.key === 'Enter') {
            // handleSubmit(); // implement this if needed
            }
        };
    ...
    // const checkAboveAmount = (event, index) => {
    //     const inputValue = Number(event.target.value);
    //     const paymentIndex = index === 's' ? 0 : index + 1;
    //     const payment = pform.payments[paymentIndex];
        
    //     console.log("Payment check:", paymentIndex);
        
    //     // Check Cash Customer due 
    //     if (form.customer_id == 1) {
    //         submitDisable.value = totalCollections.value <= Number(netAmountCalculate.value);
    //     }

    //     console.log("Input Value:", inputValue, "Net Amount:", 
    //         netAmountCalculate.value, "Payment Method:", 
    //         submitDisable.value , "Paid By:",
    //         totalCollections.value, "Due Amount:", 
    //         form.customer_id == 1 ? "Cash Customer" : "Credit Customer",
    //         payment.paid_by, "Payment Amount:", payment.amount  );
        
    //     // Check non-cash payment limit
    //     if (payment.paid_by != "cash") { 
    //         if (Number(netAmountCalculate.value) < inputValue) {
    //         event.target.value = netAmountCalculate.value;
    //         payment.amount = netAmountCalculate.value;
            
    //         if (index == 's') {
    //             pform.payments[0].amount = netAmountCalculate.value;
    //         }
            
    //         toast.warning("Not allow max amount"); 
    //         }
    //     }
    // };
    ...
    const submitDisable = computed(() => {
        if (form.customer_id == 1) {
            const collections = Number(totalCollections.value);
            const netAmount = Number(netAmountCalculate.value);
            
            // Enable when collections >= net amount
            const shouldDisable = collections < netAmount;
            
            console.log('🔐 Submit Logic:', {
            collections: collections,
            netAmount: netAmount,
            condition: `${collections} >= ${netAmount} = ${collections >= netAmount}`,
  ...
  methods: {
    ...
    handleSubmit: function () { 
      ...
      console.log('dsdd',this.invoice_info)
      ...
      if (extraMoney < 0) { 
        ...
        if(!this.checkItem('cash')){ 
           if(this.totalCollections <= Math.round(this.netAmountCalculate)){
            // Continue
           }else{
          ...
          if(checkCollectionAmount >= this.netAmountCalculate){ 
            this.$toast.warning('Collection amount violate');
            this.isSubmit = false;
            this.btn_disabled = false;
            return false;
      ...
      console.log('this.totalCollections;s', this.totalCollections);
    ...
    handleResize() {
      this.win.width = win.innerWidth;
      this.win.height = win.innerHeight - 160;
