                    ...
                    <div class="card-body">
                        <form id="purchase_order_form" @submit.prevent="submitForm()">
                            ...
                            <div class="row">
                                ...
                                <div class="form-group col-md-4">
                                    <label for="outlet_id" style="width: 100%;">Receive Location *</label>
                                    <select class="form-control border" id="outlet_id" v-model="obj.outlet_id" @change="changeOutlet($event.target.value), onkeyPress('outlet_id')" :disabled="outlet_disabled">
                                        ...
                                        <table class="table table-bordered table-centered table-nowrap w-100">
                                            <thead class="table-light">
                                                <tr class="border success item-head">
                                                    ...
                                                    <th class="text-center" style="width: 7%" v-if="obj.purchase_order_id == 'direct'">Free Qty</th>
                                                    <th class="text-center" style="width: 7%"> Weight</th>
                                                    <th class="text-center" style="width: 10%">CP</th>
                                                    <th class="text-center" style="width: 7%" v-if="obj.purchase_order_id != 'direct'">T. Qty</th>
                                            ...
                                            <tbody v-for="(product_item, i) in product_items" :key="i">
                                                <tr v-if="is_expires != 1"> 
                                                    ...
                                                    <td class="text-center" v-if="obj.purchase_order_id == 'direct'"> 
                                                        <input type="text" class="form-control" @keyup="inputChange()" @focus="addNewRow($event, i)" v-model="product_item.free_qty">
                                                    </td>                                         
                                                    <td class="text-center">
                                                    ...
                                                    <td class=" ">
                                                        ...
                                                        <!-- Expire Date Button (non-expirable, disabled) -->
                                                        <a href="#" v-if="!product_item.is_expirable" class="btn btn-primary btn-sm btnDisabled" title="Not Expirable">
                                                            <i class="fas fa-calendar-times"></i>
                                                        </a>

                                                        <!-- Expire Date Button (expirable) -->
                                                        <a href="#" v-else class="btn btn-primary btn-sm" @click.prevent="addExpireDate(product_item)" title="Add Expiry Date">
                                                            <i class="fas fa-calendar-plus"></i>
                                                        </a>

                                                        <!-- Gift Button -->
                                                        <a href="#" class="btn btn-info btn-sm" @click.prevent="addGiftItem(product_item)" title="Add Gift">
                                                            <i class="fas fa-gift"></i>
                                                        </a>

                                                        <!-- Generate Sequences Button -->
                                                        <a href="#" v-if="product_item.id" class="btn btn-info btn-sm" @click="toggleModal(product_item)" title="Generate Sequences">
                    ...
                    <div class="modal-body">
                        <div class="row">
                            <div class="form-group col-md-3 mb-3">
                                <label for="prefix1">Sequence Prefix *</label>
                                <input type="text" class="form-control" v-model="sequencePrefix" id="prefix1" placeholder="Sequence Prefix" autocomplete="off">
                            </div>

                            <div class="form-group col-md-2 mb-3">
                                <label for="size_id">Size</label>
                                <select class="form-control" v-model="size_id" id="size_id">
                            ...
                            <div class="form-group col-md-2 mb-3">
                                ...
                                <select class="form-control" v-model="color_id" id="color_id">
                            ...
                            <div class="form-group col-md-3 mb-3">
                                <label for="prefix3">Expiry Date *</label>
                                <input type="date" class="form-control" v-model="expiry_date" id="prefix3" placeholder="Expiry Date" autocomplete="off">
                            </div>
                            <div class="form-group col-md-1 mb-3">
                                <a href="#" style="margin-top: 23px;" class="btn btn-info btn-sm" @click="generateSequences(modalSequenceProduct)" title="Generate Sequences"> <i class="fas fa-code-branch"></i>
                                </a>
                            </div> 
                        </div>
                        <div class="row" v-for="(sequence, index) in modalSequenceProduct.sequences" :key="index">
                            <div class="form-group col-md-3 mb-3">
                                <input type="text" class="form-control form-control-sm" v-model="sequence.sequence" placeholder="Sequence" autocomplete="off" >
                            </div>

                            <div class="form-group col-md-2 mb-3">
                                <select class="form-control form-control-sm" v-model="sequence.size_id" id="is_expirable"> 
                                    <option v-for="(size, index) in sizes" :key="index" :value="size.id">{{ size.name }}</option>
                                </select>  
                            </div>

                            <div class="form-group col-md-2 mb-3">
                                <select class="form-control form-control-sm" v-model="sequence.color_id" id="is_expirable"> 
                                    <option v-for="(color, index) in colors" :key="index" :value="color.id">{{ color.name }}</option>
                                </select>
                            </div>

                            <div class="form-group col-md-3 mb-3">
                                <input type="date" class="form-control form-control-sm" v-model="sequence.expiry_date" placeholder="Expiry Date" autocomplete="off"  >
                            </div>
                        </div> 
...
export default {
    ...
    methods: {
        ...
        fetchAuthUser() {
            ...
            .then((res) => {
                ...
                if(res.data.data.user_roles[0].slug != "shop-manager" && res.data.data.user_roles[0].slug != "sales-man") {
                    this.outlet_disabled = false;
                    this.obj.outlet_id  = res.data.data.user.outlet_id;
                }else{
                    this.outlet_disabled = true;
                    this.obj.outlet_id  = res.data.data.user.outlet_id;
                }
            })
            .catch((err) => { 
        ...
        changeOutlet(outletid) {
            if(outletid != '') {
                this.fetchReferenceNo(outletid);
            }
        },
        // Reference Number 
        fetchReferenceNo(value='') {
            if(value != '') {
                var getEvent = axios.get(this.apiUrl+'/purchase_receives/getReferenceNo?outlet_id='+value, this.headerjson)
            }else{
                getEvent = axios.get(this.apiUrl+'/purchase_receives/getReferenceNo', this.headerjson)
            }
        ...
        fetchProductsHelper() {
            ...
            .then((res) => {
                ...
                console.log("this.suppliers", this.sizes, this.units, this.colors);
            })
            .catch((err) => { 
                this.$toast.error(err.response.data.message);
            });
        ...
        //     });
        // },
        generateSequences: async function(product) {
            const rcvQty = parseInt(product.rcv_qty);
            const generatedSequences = [];
            let suffix = 1;
            const batchSize = 10;
            console.log('Generating sequences for product:', product);

            try {
                while (generatedSequences.length < rcvQty) {
                    // Generate next batch of candidates
                    const candidates = [];
                    for (let i = 0; i < batchSize; i++) {
                        candidates.push(`${this.sequencePrefix}-${suffix + i}`);
                    } 
                    const response = await axios.post(this.apiUrl + '/check-sequences', { sequences: candidates }, this.headers);

                    const existingSequences = response.data.data.map(item => item.sequence);

                    // Add only sequences that do not exist yet
                    for (const seq of candidates) {
                        if (generatedSequences.length >= rcvQty) break;

                        if (!existingSequences.includes(seq)) {
                            generatedSequences.push({
                                product_id: product.id,
                                outlet_id: this.obj.outlet_id,
                                ...
                                purchases_price: product.purchase_price,
                            });
                        }
                    }

                    suffix += batchSize; // move suffix forward to next batch
                }

                product.sequences = generatedSequences;

                // console.log('Generated Sequences:', generatedSequences);
                this.sequencePrefix = '';
                this.$toast.success('Sequences generated successfully!');
        ...
        onChangeSupplier(event){
            if(!this.pscan_receive) {
                ...
                if(supplier_id != '') {
                    axios.post(this.apiUrl+'/purchase_receives/getPurchaseOrder', {'supplier_id': supplier_id}, this.headerjson )
                    .then((res) => {
        ...
        fetchProducts() {
            ...
            .then((resp) => {
                ...
                resp.data.data.map((item) => {
                    this.products.push({label:item.product_name+'('+item.product_code+')', value:item.id});
                });
            })
            .catch((err) => { 
                this.$toast.error(err.response.data.message);
    ...
    computed: {
        ...
        netAmount: function(){
            ...
            let net_amount = this.product_items.reduce(function(total, item){
                ...
                let item_discount = ((parseFloat(item_value) * parseFloat(item.disc_percent)) / 100);
                return total + (parseFloat(item_value) - parseFloat(item_discount)); 
            },0); 
