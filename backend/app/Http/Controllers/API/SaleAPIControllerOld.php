...
class SaleAPIController extends AppBaseController
{
    ...
    public function list(Request $request)
    {
        ...
        $from_date  = $request->input('from_date') ?? Carbon::now()->subMonths(1)->format("Y-m-d");
        $to_date    = $request->input('to_date') ?? Carbon::now()->format("Y-m-d");
        $outlet_id  = $request->input('outlet_id');

        $query = Sale::with(['salesItems', 'outlets'])->select('id','created_at', 'invoice_number', 'grand_total', 'total_amount', 'order_discount_value', 'outlet_id', 'customer_name', 'collection_amount')->where('return_type', '!=', 3)->orderBy($columns[$column], $dir);
        ...
        $sales_data = $query->withCount('salesItems')->withSum('salesItems', 'discount')->withSum('salesItems', 'mrp_price')->paginate($length);
        $return_data    = [
            'data' => $sales_data,
    ...
    public function collection(Request $request){
        ...
        if ($request->payments) {
            ...
            $payments[] = new PaymentCollection([
                ...
                'credit_card'   => $request->payments[0]['card_reference_code'],
