...
class WarehouseStockAdjustmentAPIController extends AppBaseController
{
    public function warehouseStockAdjustment(Request $request)
    {
        $stock_data = WarehouseStockProduct::find($request->stock_id);
        if(empty($stock_data)) {
        ...
        try {
            ...
            $log_insert = WarehouseStockProductLog::create($stock_log_inputs);
